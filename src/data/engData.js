// const removeTags = (str) => {
//     if ((str === null) || (str === ''))
//         return false;
//     else
//         str = str.toString();

//     // Regular expression to identify HTML tags in
//     // the input string. Replacing the identified
//     // HTML tag with a null string.
//     return str.replace(/(<([^>]+)>)/ig, '');
// }

// let string = 'Hi my name is Andrea\' +
//     'Passionate Software Engineer JavaScript Developer & Business IT Specialist';

//' + + '

/*The purpose of this page is knowledge sharing. <br />
              This can be that place where you can feel free to ask and share.
              <br />
              Programming is not just a job but can be so much more.Is that
              something that can open your mind and finally ask the right
              questions. <br />
              If you are looking for a new path or challenge you are in the
              right place. <br /> Here you can lay a solid foundation all the
              way to advanced concepts that you will be able to put into
              practice in your everyday work. <br /> Finally live in the present
              but constantly looking toward the future. <br /> For me it has
              meant not only the beginning of a new career but has opened up
              endless possibilities just waiting to be grasped. <br />
              I would be happy to be able to share this with you... <br />
              <em> it will change your life </em> */


let engData = [
    [{
        id: 1,
        //img: "https://media.istockphoto.com/id/1160791767/it/foto/cavallo-che-ride.jpg?b=1&s=170667a&w=0&k=20&c=pdXu_JNb1_RFsmqBqw__rrfbQY3sgOCKATmE03cFrNA=",
        title: "Hey, I'm Andrea ",
        text: 'Frontend Developer \n Development of user-friendly and interactive web pages \n ' +
            'JavaScript Engineer, UX UI Architect \n Turning ideas into real life is my calling'
    }],
    [{
        id: 2,
        //img: "https://c8.alamy.com/compit/c57ptc/immagine-divertente-di-un-giovane-gorilla-bloccata-il-suo-dito-medio-c57ptc.jpg",
        //title: "Scimmia Pazza",
        text: 'No standardized solutions for your services \n Development of webpages to mach your needs , \n ' +
            'Attention to details and reach the client target is my mantra. \n ' +
            'I offer web-development courses aimed at beginners, those who want to deal with programming for the first time, and experts with a desire to get even better.'
    }]
];
// {
//     id: 3,
//     img: "https://c8.alamy.com/compit/c57ptc/immagine-divertente-di-un-giovane-gorilla-bloccata-il-suo-dito-medio-c57ptc.jpg",
//     title: "Scimmia Pazza",
//     name: "Crazy Monkey",
// },
// {
//     id: 4,
//     img: "https://c8.alamy.com/compit/c57ptc/immagine-divertente-di-un-giovane-gorilla-bloccata-il-suo-dito-medio-c57ptc.jpg",
//     title: "Scimmia Pazza",
//     name: "Crazy Monkey",
// }



export default engData;